...
fn main() -> Result<(), hashi::HashiError> {

    let mut hg = hashi::HashiGrid::new(5,10)?;



    let _ = hg.add_island(hashi::Coordinate { x: 0, y: 4 })?;
    let _ = hg.add_island(hashi::Coordinate { x: 4, y: 4 })?;
    let _ = hg.add_island(hashi::Coordinate { x: 1, y: 5 })?;
    let _ = hg.add_island(hashi::Coordinate { x: 1, y: 9 })?;


    let _a = hg.bridge_islands(hashi::Coordinate { x: 0, y: 4 }, hashi::Coordinate { x: 4, y: 4 })?;
    let _a = hg.bridge_islands(hashi::Coordinate { x: 1, y: 5 }, hashi::Coordinate { x: 1, y: 9 })?;
